 ...
 * @private
 */
function _parseAttributes(attributes, nasted) {
    var properties = {};
    var required = [];
    _.forEach(attributes, function (attribute, name) {
        var property = {};
        if(attribute.type == 'json' && nasted && nasted[name]) {
          if(attribute.columnType && attribute.columnType === 'array'){
            const attributes = _parseAttributes(nasted[name], nasted);
            property[name] = {type: 'array', items: attributes}
          } else {
            property[name] = _parseAttributes(nasted[name], nasted);
          }
        } else {
