...
function _generateDefinitions(models) {
    ...
    _.forEach(models, function (model) {
        // Do tagging for  all models. Ignoring associative tables
        if (model.globalId) {
            defintions[model.identity] = parsers.attributes(model.attributes, model.nasted);
        }
    });
        ...
        _.forEach(routes, function (route) {
          ...
          var excludeParameters = route.excludeParameters || []
            ...
            var path = {
                ...
                responses: _getResponses(route.action, identity)
            };
...
function _getResponses(method, identity) {
  if (method === "create" || method === "update"){
    return {
      '200': {description: 'The requested resource', "schema": {"$ref": "#/definitions/" + identity}},
      '404': {description: 'Resource not found'},
      '500': {description: 'Internal server error'}
    }
  }
  return {
    '200': {description: 'The requested resource', "schema": {"type": "array", "items": {"$ref": "#/definitions/" + identity}} },
    '404': {description: 'Resource not found'},
    '500': {description: 'Internal server error'}
  }
}
